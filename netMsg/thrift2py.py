...
def create_package_dir(dir_name):
    """创建Python包目录，包含__init__.py文件"""
...
def main():
    # 在开始处理前，先创建所有需要的文件夹
    required_dirs = ["csMsg", "csMsgTest"]
    for dir_name in required_dirs:
        create_package_dir(dir_name)
        print(f"确保文件夹 {dir_name} 存在")
...
if __name__ == '__main__':
    folder_path = DEV_EXCEL_PATH.split("datapool/策划模板导出工具/")[0] + "tools/gen-message/messages/"
    print(folder_path)
    try:
        main()
        print("协议解析生成成功")
