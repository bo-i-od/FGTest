...
class LoginPanel(BasePage):
    ...
    def is_btn_login_active(self):
        object_id_list = self.get_object_id_list(element_data_list=[ElementsData.LoginPanel.btn_login])
        if object_id_list[0] or object_id_list[1]:
            return True
        return False

    def wait_for_btn_login(self):
        self.wait_for_appear(element_data_list=[ElementsData.LoginPanel.btn_login], timeout=20)

    def click_btn_login(self):
        self.click_element(element_data=ElementsData.LoginPanel.btn_login)
