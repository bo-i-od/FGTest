...
def get_max_same_color_ball_count_and_max_count(puzzle):
    ...
    for puzzle_split in puzzle_split_list:
        ...
        res_0 = int(res[0])
        res_1 = int(res[1])
        ball_count += res_0*res_1
        if res_0 > max_repeat_color_ball_count:
            max_repeat_color_ball_count = res_0
        if res_1 > max_count:
            max_count = res_1
    return max_repeat_color_ball_count, max_count, ball_count
...
def get_detail_list(cost_cfg):
    ...
    for item in cost_cfg:
        ...
        max_same_color_ball_count, max_count, ball_count = get_max_same_color_ball_count_and_max_count(item)
        temp_res["reward_quantity"] = int(cost_cfg[item] * reward_ratio * 0.2 + 1.83 * (max_same_color_ball_count - 1) + 0.5) * 5 * 10
        # temp_res["reward_quantity"] *= max_same_color_ball_count * 10
        temp_res["weight"] = int(max_count * 100 * (1 + 0.1 * max_same_color_ball_count))
        if ball_count == max_same_color_ball_count:
            temp_res["weight"] = temp_res["weight"] // 4
