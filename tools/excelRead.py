...
class ExcelTools:
    ...
    def get_table_data_list(self, book_name):
        table_data_list, _, _ = self.get_table_data_detail(book_name=book_name)
        return table_data_list


    def get_table_data_list_by_key_value(self, key, value, book_name=None, table_data_detail=None):
        res = []
        if value is None:
            return res
        if table_data_detail is None:
            table_data_detail = self.get_table_data_detail(book_name=book_name)
        table_data_list, structs, prefix = table_data_detail
        ...
        for table_data in table_data_list:
            if key not in table_data:
                continue
            if table_data[key] != value:
                continue
            res.append(table_data)
        return res

    def get_table_data_by_key_value(self, key, value, book_name=None, table_data_detail=None):
        if table_data_detail is None:
            table_data_detail = self.get_table_data_detail(book_name=book_name)
        table_data_list, structs, prefix = table_data_detail
        type_value = structs[prefix.upper()][key][0]
        ...
        for table_data in table_data_list:
            ...
            res = table_data
        if cur != 1:
            raise PluralElementError("表中键值的匹配项不存在或不是唯一，请使用get_table_data_list_by_key_value")
        return res
    ...
    def add_object(self, key: str = None, value=None, book_name: str = None, table_data_detail=None,
        ...
        if key and value:
            # 已经有该键值对的数据
            if self.get_table_data_list_by_key_value(key=key, value=value, table_data_detail=table_data_detail):
                print(f"{prefix}表中已经有{key}:{value}对应的数据，故跳过新增")
                return False
            if instance_object:
                json_object = instance_to_json(instance_object=instance_object)
            table_data_list.append(json_object)
    ...
    def remove_object(self, key: str, value, book_name: str = None, table_data_detail=None, json_object: dict = None,
        ...
        table_data_list, structs, prefix = table_data_detail
        cur = 0
        while cur < len(table_data_list):
            table_data = table_data_list[cur]
            if key not in table_data:
                cur += 1
                continue
            if value != table_data[key]:
                cur += 1
                continue
            if instance_object:
                json_object = instance_to_json(instance_object=instance_object)
    ...
    def change_object(self, key: str, value, book_name: str = None, table_data_detail=None, json_object: dict = None,
        ...
        while cur < len(table_data_list):
            ...
            if instance_object:
                json_object = instance_to_json(instance_object=instance_object)
            table_data_list[cur] = json_object
            cur += 1
        self.write_data_txt(name=prefix, json_object_list=table_data_list)
...
class ExcelToolsForActivities(ExcelTools):
    ...
    def group_id_to_timer_id(self, group_id):
        ...
        """
        table_data = self.get_table_data_by_key_value(book_name="MISSION_GROUP.xlsm", key="groupId", value=group_id)
        timer_id = table_data["openArg"]
        return timer_id
